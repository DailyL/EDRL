...
from env.make_env import make_tudrl_purestate, make_vec_env, make_vec_env_purestate, make_srl_purestate, make_real_env
...
def train(config_path, env, agent_name: str, cost_limit=10):
    """Main training loop."""
    ...
    print(f"Using {agent_name} with cost limit {cost_limit}")
    ...
    for total_steps in range(c.timesteps):
        ...
        # memorize
        if c.Env.name == "SRL_Traj_Env":
            agent.memorize(s, a, r, s2, d, info)
        else:
            agent.memorize(s, a, r, s2, d, info)
        ...
        if (total_steps + 1) % c.epoch_length == 0 and (total_steps + 1) > c.upd_start_step:
            ...
            if agent.is_multi:
                ...
                agent.logger.log_tabular("Eval_cost_per_step", with_min_and_max=True)
                agent.logger.log_tabular("Q_val", with_min_and_max=True)
                agent.logger.log_tabular("Cost_Q_val", with_min_and_max=True)
                agent.logger.log_tabular("Critic_loss", average_only=True)
                agent.logger.log_tabular("Cost_Critic_loss", average_only=True)
                agent.logger.log_tabular("Actor_loss", average_only=True)
                agent.logger.log_tabular("Lambda", average_only=True)
                agent.logger.log_tabular("Cost_Critic_LR", average_only=True)
                agent.logger.log_tabular("Critic_LR", average_only=True)
...
def test(config_path, env, agent_name: str):
    ...
    c.critic_cost_weights = f"experiments/{agent_name}_{c.Env.name}__{date}_{seed}/{agent_name}_critic_best_weights.pth"
...
def main(algo, train_mode, cost_fn_mode, cost_limit=10):

    env = make_real_env(cost_fn_mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-cl", "--cost_limit", help="Cost limit", default=10)
    ...
    main(algo, train_mode, cost_fn_mode, cost_limit)
